...
export default function Project({
  ...
  tags,
  imageUrl,
  url, // Destructure the `url` from props
}: ProjectProps) {
  ...
  return (
    ...
    >
      <section className="bg-gray-100 max-w-[42rem]  border border-black/5 rounded-lg overflow-hidden sm:pr-8 relative sm:h-[23rem] hover:bg-gray-200 transition sm:group-even:pl-8 dark:text-white dark:bg-white/10 dark:hover:bg-white/20">
        <div className=" relative pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[60%] flex flex-col h-full sm:group-even:ml-[18rem]">
          ...
          {url && (
            <Link href={url} passHref>
              <button className="mt-7 absolute top-1 right-10 text-[0.8rem] cursor-pointer bg-blue-500  text-white font-semibold py-2 px-4 rounded-full  transition dark:text-white/70 hover:bg-blue-600">Visit </button>
            </Link>
          )}
          </div>
        ...
        group-even:right-[initial] group-even:-left-[0rem]
        group-even:w-[20rem]"
